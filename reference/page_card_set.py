    ...
    def clear_child(self):
        while self.v_layout.count():
            ...
            if widget is not None:
                ...
                del item
class cards_Window(BaseChildWindow):
    def __init__(self, parent=None,title="", width=600, height=200,content="?",callback=None):
        ...
        self.changed_key = []
        self.changed_relect = []
        self.path = registry["current_page_id"]+"_cards"+content
        self.had = config.deep_check(self.path)
        print(self.had,"lushifzun")
        if self.had:
            self.settings = config.get(self.path)
        else:
            self.settings = copy.deepcopy(config.get("moren_cards"))
        # 右侧容器（你的自定义容器）
