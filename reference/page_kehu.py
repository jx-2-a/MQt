...
class UserDetailWindow(BaseChildWindow):
    def __init__(self, user_data=None, records=None, title="用户详情",
        ...
        if user_data :
            ...
            self.user_data = registry["data_libary"].get_customer_by_id(user_data)
        else:
            self.user_data ={}
    ...
    def _create_user_fields(self):
        ...
        # 单字段
        # 普通模式，允许加号
        self.input_name = MultiListField("姓名",[self.name], allow_add=False)
        self.input_title = MultiListField("称号", [self.user_data.get("title", "")], allow_add=False)
        self.input_wechat = MultiListField("微信",[ self.user_data.get("wechat", "")], allow_add=False)
        self.input_note = MultiListField("备注", [self.user_data.get("note", "")], allow_add=False)
        ...
        # 多字段：电话、车牌
        phone_values = self.user_data.get("phones")
        if phone_values is None and self.user_data.get("phone"):
            phone_values = [self.user_data.get("phone")]
        self.multi_phone = MultiListField("电话", phone_values or [])

        plate_values = self.user_data.get("plates")
        if plate_values is None and self.user_data.get("plate"):
            plate_values = [self.user_data.get("plate")]
        self.multi_plate = MultiListField("车牌", plate_values or [])

        self.form_layout.addWidget(self.multi_phone)
        self.form_layout.addWidget(self.multi_plate)
        if not self.new_mode:
            self._create_record_button(self.form_layout)
            self._create_baoxiu_button(self.form_layout)
    ...
    def _create_baoxiu_button(self,outer_layout):
        self.btn_record = QPushButton("查询保修记录")
        self.btn_record.setFixedHeight(30)
        self.btn_record.clicked.connect(self._on_query_baoxiu_records)
    ...
    def _on_ok(self):
        ...
        data = {
            "name": self.input_name.get_values(),
            "title": self.input_title.get_values(),
            "wechat": self.input_wechat.get_values(),
            "note": self.input_note.get_values(),
            "phones": self.multi_phone.get_values(),
            "plates": self.multi_plate.get_values(),
        }
        if self.new_mode:
            registry["data_libary"].insert_customer(data)
        else:
            registry["data_libary"].update_customer(data,self.user_id)
        if self.callback:
            # new_mode 走新增逻辑，查看模式走更新逻辑都可用同一回调
            self.callback(data["name"][0])
        self._close_self()
