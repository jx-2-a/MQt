...
from app.framework.config_manager import load_config, load_all_layouts
...
_ATTR_FORMAT = {
    ...
    "font_size": "14px", "font_weight": "bold", "font_style": "normal",
    "border": "1px solid #333", "border_top": "1px solid #333",
    "border_right": "1px solid #333", "border_bottom": "1px solid #333",
    "border_left": "1px solid #333", "border_radius": "8px",
    "padding": "4px", "margin": "4px",
    "width": "100px", "height": "32px",
    "min_width": "100px", "max_width": "800px",
    "min_height": "32px", "max_height": "600px",
    "selection_background_color": "#007acc", "selection_color": "#ffffff",
    "outline": "none", "text_align": "center",
    "subcontrol_origin": "margin", "subcontrol_position": "top",
    "spacing": "6px", "left": "4px",
}
...
class StyleController(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self._prop_container.setAttribute(Qt.WA_StyledBackground, True)
        self._prop_container.setAutoFillBackground(False)
    ...
    def _build_prop_panel(self, style_dict, inherited):
        ...
        if len(sub_controls) > 1:
            sc_row = QHBoxLayout()
            sc_row.setSpacing(6)
            sc_row.addWidget(QLabel("子样式:"))
            sc_combo = QComboBox()
            for sc_key in sub_controls:
                sc_combo.addItem(subcontrol_display_name(sc_key, self._current_widget_type), sc_key)
            ...
            sc_combo.currentIndexChanged.connect(lambda i: self._on_subcontrol_changed(sc_combo.itemData(i)))
    ...
    def _add_text_field(self, form, key, label, inh_text):
        ...
        row.setSpacing(4)
        edit = QLineEdit()
        fmt = _ATTR_FORMAT.get(key, "")
        edit.setPlaceholderText(f"例: {fmt}" if fmt else "")
    ...
    def _add_combo_field(self, form, key, label, items, inh_text):
        ...
        combo.currentTextChanged.connect(
            lambda v, k=key: self._on_prop_changed(k, v if v != "（未设置）" else ""))
    ...
    def _pick_color(self, key, edit, preview):
        cur = edit.text().strip()
        init_color = parse_color(cur) or QColor(Qt.white)
        dlg = ColorPicker(init_color, self)
        if dlg.exec_() == ColorPicker.Accepted:
            color = dlg.selected_color()
            edit.setText(color_to_qss(color))
            self._update_color_preview(key, edit.text().strip())

    def _update_color_preview(self, key, value):
        preview = self._color_previews.get(key)
        if not preview:
            return
        preview.set_color(parse_color(value))
    ...
    def apply_styles(window_key):
        ...
        if window_key in cfg:
            ...
            from app.windows.base_window import BaseWindow
            for w in QApplication.topLevelWidgets():
                if isinstance(w, BaseWindow) and w.config_key == window_key:
    ...
    def update_window(window):
        from app.windows.base_window import BaseWindow
        key = window.config_key if isinstance(window, BaseWindow) else "main"
